...
using parse_types = boost::mpl::list<
    ...
    std::string, boost::mpl::list<const void *, std::size_t>, std::istream,
    boost::filesystem::path>;

using serialize_types =
    boost::mpl::list<google::protobuf::io::ZeroCopyOutputStream, std::ostream,
...
BOOST_AUTO_TEST_CASE_TEMPLATE(parse_partial_only_optional, Stream,
                              parse_types) {
  Message msg;
  stream<Stream> ss(only_optional_data);
  ss.parse_partial(msg);
...
BOOST_AUTO_TEST_CASE_TEMPLATE(serialize_partial_optional, Stream,
                              serialize_types) {
  stream<Stream> ss;
...
BOOST_AUTO_TEST_CASE(append_partial_empty) {
  ...
  bp_io::append_partial(empty, buffer);
...
BOOST_AUTO_TEST_CASE(append_partial_only_optional) {
  std::string buffer = prefix;
  bp_io::append_partial(only_optional, buffer);
  BOOST_CHECK(boost::algorithm::starts_with(buffer, prefix));
...
BOOST_AUTO_TEST_CASE(append_empty) {
  std::string buffer = prefix;
  BOOST_CHECK_THROW(bp_io::append(empty, buffer), bp::serialize_error);
}

BOOST_AUTO_TEST_CASE(append) {
  std::string buffer = prefix;
  bp_io::append(multiple, buffer);
  BOOST_CHECK(boost::algorithm::starts_with(buffer, prefix));
  BOOST_CHECK_EQUAL(buffer.substr(prefix.size()), multiple_data);
}

BOOST_AUTO_TEST_CASE(to_string_partial) {
  BOOST_CHECK_EQUAL(bp_io::to_string_partial(empty), empty_data);
  BOOST_CHECK_EQUAL(bp_io::to_string_partial(only_required),
                    only_required_data);
  BOOST_CHECK_EQUAL(bp_io::to_string_partial(only_optional),
                    only_optional_data);
  BOOST_CHECK_EQUAL(bp_io::to_string_partial(optional), optional_data);
  BOOST_CHECK_EQUAL(bp_io::to_string_partial(single), single_data);
  BOOST_CHECK_EQUAL(bp_io::to_string_partial(multiple), multiple_data);
}

BOOST_AUTO_TEST_CASE(to_string) {
  BOOST_CHECK_THROW(bp_io::to_string(empty), bp::serialize_error);
  BOOST_CHECK_EQUAL(bp_io::to_string(only_required), only_required_data);
  BOOST_CHECK_THROW(bp_io::to_string(only_optional), bp::serialize_error);
  BOOST_CHECK_EQUAL(bp_io::to_string(optional), optional_data);
  BOOST_CHECK_EQUAL(bp_io::to_string(single), single_data);
  BOOST_CHECK_EQUAL(bp_io::to_string(multiple), multiple_data);
}
