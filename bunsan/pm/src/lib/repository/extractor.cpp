...
namespace {
bool out_of_range(const std::time_t now, const std::time_t lifetime,
                  const std::time_t last_write_time) {
  return now > lifetime + last_write_time || last_write_time > lifetime + now;
}
}  // namespace
...
void repository::extractor::extract_source(
...
void repository::extractor::extract_installation(
    const entry &package, const boost::filesystem::path &destination) {
  try {
    const tempfile tmp = local_system_().tempdir_for_build();
...
namespace {
void merge_dir(const boost::filesystem::path &source,
               const boost::filesystem::path &destination) {
  ...
  try {
    ...
    BOOST_THROW_EXCEPTION(extractor_merge_directories_error()
                          << extractor_merge_directories_error::source(source)
                          << extractor_merge_directories_error::destination(
                                 destination) << enable_nested_current());
  }
}
}  // namespace

void repository::extractor::merge_directories(
...
repository::cache &repository::extractor::cache_() { return m_self.cache_(); }

repository::local_system &repository::extractor::local_system_() {
