...
repository::distributor::distributor(repository &self,
  ...
  if (!(m_archiver =
            format().archiver.instance_optional(local_system_().resolver()))) {
    BOOST_THROW_EXCEPTION(
        invalid_configuration_remote_archiver_error()
        << invalid_configuration_remote_archiver_error::utility_type(
            format().archiver.type));
  }
  if (!(m_fetcher =
            m_config.fetcher.instance_optional(local_system_().resolver()))) {
    BOOST_THROW_EXCEPTION(invalid_configuration_fetcher_error()
                          << invalid_configuration_fetcher_error::utility_type(
                              m_config.fetcher.type));
  }
}

const format_config &repository::distributor::format() const {
  return m_config.format;
}

bunsan::utility::archiver &repository::distributor::archiver() {
  return *m_archiver;
}
...
void repository::distributor::update_file(const std::string &url,
                                          const boost::filesystem::path &file,
                                          const std::string &checksum) {
  const auto outdated = [](const boost::filesystem::path &file,
                           const std::string &checksum) {
    return !boost::filesystem::exists(file) || pm::checksum(file) != checksum;
  };
