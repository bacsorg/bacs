...
p 1)EOF");
  ...
  BOOST_CHECK(
      executor(pt).add_argument("2", "6", "4").context().built().arguments() ==
      out_2);
  ss in_3(R"EOF(
t 1       ; 1
c         ; 234
...
d
{
  ...
  vs out_5 = out_4;
  BOOST_CHECK(
      executor(pt).add_argument("6", "8", "5").context().built().arguments() ==
      out_5);
  ss in_6(R"EOF(
t 1
t 2
