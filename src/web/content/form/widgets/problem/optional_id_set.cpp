...
optional_id_set::optional_id_set(const std::string &flag_,
  ...
  // std::string flag_id(16, '\0');
  {
    ...
    std::generate(ids_id.begin(), ids_id.end(),
                  [&rng, &rnd]() { return rnd(rng); });
    // std::generate(flag_id.begin(), flag_id.end(), [&rng, &rnd](){return
    // rnd(rng);});
  }
  flag.name("ids_flag");
  // flag.id(flag_id);
  flag.message(flag_);
  const std::string flag_head =
      " onclick=\"document.getElementById('" + ids_id + "').disabled = ";
...
bool optional_id_set::validate() {
  ...
  switch (flag_type) {
    case flag_enables:
      if (flag.value()) valid = valid && ids.validate();
      break;
    case flag_disables:
      if (!flag.value()) valid = valid && ids.validate();
      break;
...
boost::optional<archive::problem::IdSet> optional_id_set::value() {
  switch (flag_type) {
    case flag_enables:
      if (flag.value()) return ids.value();
      break;
    case flag_disables:
      if (!flag.value()) return ids.value();
      break;
...
void optional_id_set::value(
    const boost::optional<archive::problem::IdSet> &id_set_) {
  ids.disabled(!id_set_);
  if (id_set_) ids.value(id_set_.get());
  switch (flag_type) {
    case flag_enables:
