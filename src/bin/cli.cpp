 ...
 public:
  ...
  void initialize_argument_parser(argument_parser &parser) override {
    ...
    parser.add_options()(
        ...
        "package,p", value<std::string>(&package), "Package name"
    )(
        "extract,e", value<std::string>(&extract_to),
        "Extract package to location"
    )(
        "install,i", value<std::string>(&install_to),
        "Install package to location"
    )(
        "update,u", value<std::string>(&update_installation),
        "Update installed package"
    )(
        "lifetime,l", value<std::time_t>(&lifetime),
        "Only check for update if lifetime has passed since previous attempt"
    )(
        "create,r", value<std::string>(&create),
        "Create source package from source"
    )(
        "create-recursively,R", value<std::string>(&create_recursively),
        "Create source package from source"
    )(
  ...
  int main(const variables_map &variables) override {
    ...
    if (variables.count(
            "initialize")) {  // initialize before repository construction
    ...
    if (variables.count("clean")) {  // clean before working with packages
      BUNSAN_LOG_INFO << "Attempt to clean repository";
      repo.clean_cache();
    }
    if (variables.count("package")) {
      if (variables.count("extract")) {  // extracting
      ...
      BUNSAN_LOG_INFO << "Attempt to create source packages recursively "
                         "from source starting from \"" << create_recursively
                      << "\"";
      repo.create_recursively(create_recursively, variables.count("strip"));
    }
    return exit_success;
  }
