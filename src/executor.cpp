...
bunsan::utility::executor::executor(const boost::property_tree::ptree &command): next_positional(0)
{
    BUNSAN_LOG_TRACE << "creating executor object from boost::property_tree::ptree";
...
class bunsan::utility::executor::ref_visitor: public boost::static_visitor<std::string>
{
public:
    ...
    {
        BUNSAN_LOG_TRACE << "named: returning std::string=\"" <<
                            named->at(key) << "\" from std::string=\"" << key << "\"";
        return named->at(key);
    }
    std::string operator()(size_t index)
    {
        BUNSAN_LOG_TRACE << "positional: returning std::string=\"" <<
                            positional->at(index).get() << "\" from size_t=\"" << index << "\"";
...
class bunsan::utility::executor::token_visitor: public boost::static_visitor<std::string>
{
public:
...
bunsan::process::context bunsan::utility::executor::context() const
{
    ...
    if (iter != named.end())
        ctx.use_path(iter->second == "true" ||
                     iter->second == "yes" ||
                     iter->second == "1"); // TODO bad code
    return ctx;
}

int bunsan::utility::executor::sync() const
{
    BUNSAN_LOG_TRACE << "trying to execute";
