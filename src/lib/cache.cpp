...
namespace bunsan{namespace pm
{
    namespace
    {
        ...
        {
            config.name.installation.data = installation_data;
            return config;
        }
    }
    ...
    {
        const boost::filesystem::path root = m_config.root / package.location() / installation_subdir;
        if (m_repository.need_update(package, root, m_config.lifetime))
        {
            const boost::unique_lock<boost::shared_mutex> lk(m_lock);
            const boost::filesystem::file_status status = boost::filesystem::symlink_status(root);
            switch (status.type())
            {
            default:
                clean_();
