...
namespace bacs{namespace archive
{
    ...
    namespace
    {
                            ...
                            Args &&...args)
        {
            ...
            {
                (*map.mutable_entry())[id] = convert(
                    (this_->*get)(id, std::forward<Args>(args)...)
                );
            }
            return map;
        }
        ...
        problem::IdSet get_all_set(repository *const this_,
                                   bool (repository::*get)(const problem::id &, Args...),
                                   const problem::IdSet &id_set,
                                   Args &&...args)
        {
    ...
    problem::ImportMap repository::insert_all(const boost::filesystem::path &location)
    {
    ...
    problem::ImportMap repository::insert_all(const archiver_options &archiver_options_,
    ...
    bunsan::tempfile repository::extract_all(const problem::IdSet &id_set,
                                             const archiver_options &archiver_options_)
    {
