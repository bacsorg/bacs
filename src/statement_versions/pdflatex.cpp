...
namespace bacs{namespace problem{namespace statement_versions
{
    ...
    namespace
    {
        ...
        boost::filesystem::path get_source(
            const boost::filesystem::path &source)
        {
            return source.filename();
        }
        boost::filesystem::path get_target(
            const boost::filesystem::path &source)
        {
        ...
        const bunsan::pm::entry &resources_package) const
    {
        try
        {
            ...
            index.source.import.source.insert(
                std::make_pair("src", resources_package)
            );
