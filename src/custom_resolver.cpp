...
namespace bunsan{namespace utility
{
        ...
        while (iter != m_config->alias.end())
        {
            ...
            if (used.find(name) != used.end())
                BOOST_THROW_EXCEPTION(
                    resolver_circular_alias_dependencies_error());
            used.insert(name);
        }
    }
    ...
    void custom_resolver::apply_path(boost::filesystem::path &name) const
    {
        ...
        if (name != name.filename())
            return;
        if (m_config->path.find(name) != m_config->path.end())
            apply_back(name);
    }
        ...
        const boost::filesystem::path &exe)
    {
        ...
        {
            BOOST_THROW_EXCEPTION(
                resolver_find_executable_error() <<
...
#warning unimplemented
    ...
    std::unique_ptr<resolver> custom_resolver::clone() const
    {
        ...
        if (m_config)
        {
            return std::make_unique<custom_resolver>(
                *m_config,
